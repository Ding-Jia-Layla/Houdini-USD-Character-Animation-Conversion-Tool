...
def get_skeleton_data(fbx_node):
    ...
    for index, value in enumerate(joints):
        ...
        if(len(parts)==1):
            ...
            child = parts[-1]
            parent = parts[-2]
            T = bind_transform_dict[child] * bind_transform_dict[parent].GetInverse()
            M = Gf.Matrix4d(1.0)
...
def export_skeleton(stage,fbx_node,skel):
    ...
    joints, bindTransforms, restTransforms,root_name = setup_skeleton(joints, bindTransforms, restTransforms,geom_bindTransform, root_name,skel)
    #return skeleton
